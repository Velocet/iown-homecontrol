...
function IOSettingsUtils.baseCommands.writePrivate(params, currentRawStates, context)
	...
	return bytearray(params[1]), {type = 2415919090} -- 0x8FFFFFF2
end
function IOSettingsUtils.baseCommands.writePrivateWithoutPolling(params, currentRawStates, context)
	...
	return bytearray(params[1]), {type = 49414657}
end
function IOSettingsUtils.baseCommands.readPrivate(params, currentRawStates, context)
	...
	return bytearray(params[1]), {access = "r",type = 2415919090}
end
...
function IOSettingsUtils.baseCommands.activateStatusRequest(params, currentRawStates, context)
	...
	return bytearray(params[1]), {type = 65554}
end
...
function IOSettingsUtils.baseCommands.readMP(params, currentRawStates, context)
	...
	return IOUtils.readActivateFunc({}), {access = "r",type = 65554}
end
...
function IOSettingsUtils.baseCommands.deadMan(params, currentRawStates, context)
	...
	local payload = bytearray({2,0,1,0,1,0})
	payload:writeInt8(2, params[1])
	payload:writeInt8(4, durationMultiplier)
	local timeout = 2000
	if context ~= nil and context.keepAliveTimeout ~= nil then timeout = context.keepAliveTimeout end
	return payload, {type = 2415919091,keepAliveTimeout = timeout}
end
function IOSettingsUtils.baseCommands.getData(params, currentRawStates, context)
	...
	local payload = bytearray({2,236,0,0})
	payload:writeInt16(3, params[1])
	return payload, {access = "r",type = 49414657}
end

IOSettingsUtils.conversions = {}
function IOSettingsUtils.conversions.simpleMatching(rawValue, params)
...
function IOSettingsUtils.conversions.readSOFOPU(rawValue, params)
	IOSettingsUtils.assertParamCount(params, 1, "readSOFOPU")
	return IOSomfyUtils.getParamValue(rawValue, params[1])
end
function IOSettingsUtils.conversions.extractBytesFromAck(rawValue, params)
	...
	if     length == 1 then return rawValue:readInt8(startPosition + 1)
	elseif length == 2 then return rawValue:readInt16(startPosition + 1)
	elseif length == 3 then return rawValue:readInt24(startPosition + 1)
	elseif length == 4 then return rawValue:readInt32(startPosition + 1)
	end
	error("Invalid parameters for extractBytesFromAck")
end
function IOSettingsUtils.conversions.rangeConversion(rawValue, params)
...
function IOSettingsUtils.conversions.replaceBytes(rawValue, params)
...
function IOSettingsUtils.triggerFunctions.refreshState(params, statesDefinitions, commandParams, states)
	local payloads = {}
	local mpfps
	local index = 1
...
function IOSettingsUtils.runConfig(commandName, commandParams, commandDefinitions, statesDefinitions, envContext)
	...
	if commandDefinition == nil then error("Unknown command name : " .. commandName) end
	...
	for i in ipairs(commandDefinition) do
		...
		if context.context == nil or IOSettingsUtils.isContextValid(context.context, commandParams, states) then
			...
			if context.trigger ~= nil then
				...
				for _, v in ipairs(refreshStateCommands) do table.insert(commands, v) end
			end
...
function IOSettingsUtils.computeCommand(baseCommand, definitionParams, conversions, commandParams, states, rawStates, envContext)
	if baseCommand == nil then error("Base command should not be null")
	elseif IOSettingsUtils.baseCommands[baseCommand] == nil then error("Unknown base command : " .. baseCommand) end
	local params = IOSettingsUtils.convertParamsValue(definitionParams, commandParams, states)
	if conversions ~= nil then params = IOSettingsUtils.applyParamsConversions(params, conversions, commandParams, states) end
...
function IOSettingsUtils.aggregateConfig(statesDefinitions)
	...
	for k, v in pairs(statesDefinitions) do
		states[k] = v.value
		if v.conversions ~= nil then states[k] = IOSettingsUtils.applyStateConversions(states[k], v.conversions) end
		if v.type == "int" then states[k] = tonumber(states[k]) end
	end
	return states
end
		...
		for k, v in pairs(statesDefinitions) do
			...
			if v.refreshAll == true then
				if stateDefinition.baseCommand == "readFP" then
					...
					if stateDefinition.conversions ~= nil then refreshParams = IOSettingsUtils.applyParamsConversions(refreshParams, stateDefinition.conversions, {}, states) end
					for key in ipairs(refreshParams) do mpfps[#mpfps + 1] = refreshParams[key] end
				elseif stateDefinition.baseCommand ~= "readMP" then
					local payload, options = IOSettingsUtils.computeCommand(stateDefinition.baseCommand, stateDefinition.params, stateDefinition.conversions, {}, states)
					if IOSettingsUtils.isCommandAlreadyPresentInArray(payloads, payload, options) == false then
						payloads[index] = payload
						payloads[index + 1] = options
						index = index + 2
