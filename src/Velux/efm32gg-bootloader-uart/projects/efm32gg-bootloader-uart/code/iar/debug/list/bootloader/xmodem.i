...
__ramfunc void AesCtr128(const uint8_t* key,
               ...
               uint8_t*               ctr,
               AES_CtrFuncPtr_TypeDef ctrFunc,
               _Bool initDecryption             );
__ramfunc _Bool AesFinished(void);
...
static uint8_t rawPacket[2][(((sizeof(XMODEM_packet))+((4)-1))&(~((4)-1)))];


static uint8_t counterValue[16]; 
...
__ramfunc inline int XMODEM_verifyPacketChecksum(XMODEM_packet *pkt, int sequenceNumber)
{
  uint16_t packetCRC;
...
__ramfunc void XMODEM_download(uint32_t baseAddress, uint32_t endAddress, _Bool encrypted)
{
  ...
  _Bool          initDecryption = 1;
  _Bool          readCounterFromFile = 1;

  for (addr = baseAddress; addr < endAddress; addr += 4096U)
  {
  ...
  {    
	...
	((CMU_TypeDef *) (0x400C8000UL))->HFCORECLKEN0 |= (0x1UL << 1);
  }
  ...
  while (1)
  {
	...
	{
	  if (((USART_TypeDef *) (0x4000C000UL))->STATUS & (0x1UL << 7))
	  {
...
xmodem_transfer:
  ...
  while (1)
  {
	...
	if(encrypted && readCounterFromFile)
	{
		...
		{   
			...
			counterValue[i] = rawPacket[sequenceNumber & 1][i+4];
		}
		readCounterFromFile = 0;
		sequenceShift = 2;
	}
	else
	{        
		...
		FLASH_writeBlock((void *)baseAddress,
						 (sequenceNumber - sequenceShift) * 128,
						  128,
  ...
  while (((DMA_TypeDef *) (0x400C2000UL))->CHENS & (0x1UL << 0));
