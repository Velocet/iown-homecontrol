...
export class EquipementPropertiesService {
    ...
    private functions = {
        all: {
            ...
            RSSI_LEVEL: {read: (val) => {switch (true) {
                        case val > 60:return 'EXCELLENT';
                        case val <= 60 && val > 20:return 'BON';
                        case val <= 20:return 'FAIBLE';
                    }
                },
            },
            EQUIPMENT_AVAILABLE: {read: (val) => {if (val === true || val === 'true') {return 'AVAILABLE';} else {return 'UNAVAILABLE';}},},
            EQUIPMENT_DETAIL_STATE_BACK_IMPULSE: {read: (val) => {if (isInteger(val) && val > 0) {return 'YES';}return 'NO';},},
            labelChoice: (val, label) => `${label}_${val}`,
            EQUIPMENT_DETAIL_IO_PROFIL: {read: (val) => {if (isArray(val)) {return val[0];} else {return val;}},},
            EQUIPMENT_DETAIL_LATEST_MOTOR_STOP: {read: (val) => {
                    const supportedVals = [
                        '1',
                        '49152',
                        '49153',
