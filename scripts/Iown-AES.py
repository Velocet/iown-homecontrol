...
This is an exercise in secure symmetric-key encryption, implemented in pure Python (no external libraries needed).

Original AES implementation by Bo Zhu (about.bozhu.me) at github.com/bozhu/AES-Python.
PKCS#7 padding, CBC mode, PKBDF2, HMAC, byte array and string support added by github.com/boppreh/aes. 
Other block modes contributed by @righthandabacus.
Although this is an exercise, the `encrypt` and `decrypt` functions should provide reasonable security to encrypted messages.
"""
...
def matrix2bytes(matrix):
    """Convert 4x4 matrix into 16-byte array."""
    return bytes(sum(matrix, []))

def xor_bytes(a, b):
    """Return new byte array with the elements xor'ed."""
    return bytes(i^j for i, j in zip(a, b))

def inc_bytes(a):
    """Return new byte array with the value increment by 1 """
    out = list(a)
...
def pad(plaintext):
    """Pad plaintext with PKCS#7 padding to a multiple of 16 bytes.
    Note: If plaintext size is a multiple of 16, a whole block will be added.
    """
...
def unpad(plaintext):
    """Remove PKCS#7 padding and return unpadded text, ensuring the padding was correct."""
...
class AES:
    ...
    def encrypt_custom(self, plaintext, iv):
        assert len(iv) == 16
        ...
        for plaintext_block in split_blocks(plaintext):
            # CFB mode encrypt: plaintext_block XOR encrypt(prev_plaintext)
            ciphertext_block = xor_bytes(plaintext_block, self.encrypt_block(prev_plaintext))
            blocks.append(ciphertext_block)
            prev_plaintext = plaintext_block

        return b''.join(blocks)

    def decrypt_custom(self, ciphertext, iv):
        assert len(iv) == 16

        blocks = []
        prev_plaintext = iv
        for ciphertext_block in split_blocks(ciphertext):
            # CFB mode decrypt: ciphertext XOR decrypt(prev_plaintext)
            plaintext_block = xor_bytes(ciphertext_block, self.encrypt_block(prev_plaintext))
